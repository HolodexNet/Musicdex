...
export const hotFormatter: PlaylistFormatter<{ ch: string }, undefined> = {
  bannerImage: (playlist, _, data) => {
    return `https://holodex.net/statics/channelImg/${playlist.art_context?.channels?.[0]}/banner/3.jpeg`;
  },
  channelImage: (playlist, _, data) => {
    return `https://holodex.net/statics/channelImg/${playlist.art_context?.channels?.[0]}.png`;
  },
